...
#### Scenario: Fallos de renderizado no afectados por una regeneración restringida a un tipo
- **WHEN** una entrada de tipo `page` tiene un fallo de renderizado registrado y se ejecuta `wp wpasl generate --all --post-type=page`
- **THEN** la marca de generación de esa página se olvida pero su contador de fallos de renderizado permanece sin cambios
