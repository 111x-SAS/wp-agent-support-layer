---
name: ship-change
description: Run this repo's full plan-to-merge pipeline for a change - openspec proposal (written by a fable-model agent), a review gate, an independent Orca worktree running Claude opusplan to implement and open a PR, an automated code-review pass on the PR with the same agent auto-fixing and re-verifying findings in that worktree, a second review gate, then squash-merge, openspec spec sync + archive, and worktree cleanup. Use when the user wants to ship a change "the way we did it before" / "con el mismo proceso" / "/ship-change <description>" - not for a quick one-off edit that doesn't need a formal plan or PR.
...
6. **Automated code review** (review early, catch what tests/phpcs don't - logic bugs, reuse/simplification, efficiency): invoke the `code-review` skill against the PR number, e.g. `/code-review <n> high --comment`. No new worktree for this - targeting a PR number works over the remote diff via `gh`, so run it yourself (the orchestrator) from wherever you already are. Default to `high` effort for anything touching security-sensitive surfaces (writing files, auth, external requests) or `medium` for routine changes; never invoke the billed `ultra` tier without the user asking for it by name. Let it post findings as inline PR comments (`--comment`).
7. **Auto-fix + re-verify, in the same implementation worktree** - only if step 6 found anything: send a follow-up prompt to the *same* agent/terminal from step 3 (it's still there; don't spin up a third worktree - that would repeat `composer install`/env setup for no reason) telling it to read the review findings (the PR's inline comments, or paste the findings list), apply fixes, re-run the same verification commands from step 4 (tests, linter, `openspec validate <name> --strict`) for real, and push a follow-up commit (Conventional Commits, repo-user authorship only, same rules as step 4). Wait for it the same way as step 5. **This never substitutes Gate 2** - it only means Gate 2 happens after the fixed, re-verified state instead of the first draft. If step 6 found nothing, skip straight to Gate 2.
8. **Gate 2**: report the PR URL plus a summary of what the automated review found and (if applicable) what got auto-fixed and re-verified, and ask the user to review it. Stop - do not merge, sync, or archive until they say so. The user approving here approves the merge; it is never the agent's own call.
...
- Auto-fixing code-review findings (Phase 2 step 7) is a quality step, not an approval step - it never substitutes Gate 2, and the agent never approves its own work for merge. Only the user's explicit go-ahead at Gate 2 does that.
