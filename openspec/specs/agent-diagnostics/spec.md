...
- **THEN** todas las peticiones realizadas son `GET` a la portada, la entrada de muestra, `/robots.txt`, `/llms.txt`, los archivos por tipo, `/auth.md`, `/agent-skills.json`, `/.well-known/api-catalog` y la sonda de almacenamiento, y ninguna apunta a un endpoint de registro o autenticación
