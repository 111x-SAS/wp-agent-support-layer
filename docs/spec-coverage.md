...
| Primera publicación no invalida nada (sin lectura/escritura de estado) | `Test_Runner::test_publishing_a_brand_new_post_does_not_write_state` | A |
| Edición concurrente de una entrada ya generada en un ciclo anterior (no se resucita al guardar la ejecución) | `Test_Runner::test_concurrent_edit_of_an_older_item_is_not_resurrected_when_the_run_saves` | A |
| `State::save()`: un llamador que solo pasa el id que tocó no resucita una entrada distinta eliminada concurrentemente (el merge la reconstruye desde una lectura fresca) e ignora un caché de opción local desactualizado (`wp_cache_delete()` propio, no basta con `load()`) | `Test_State::test_a_caller_passing_only_its_own_touched_id_does_not_resurrect_a_concurrent_removal`, `test_save_merge_bypasses_a_stale_local_option_cache` | A |
| Edición concurrente de otra entrada durante un relleno diferido (`mark_generated()` no resucita un id distinto invalidado mientras tanto) | `Test_State::test_mark_generated_does_not_resurrect_a_concurrently_forgotten_item` | A |
| Edición concurrente al iniciar un ciclo por WP-CLI restringido a un tipo | `Test_Runner::test_run_cycle_with_post_types_does_not_resurrect_a_concurrently_forgotten_item` | A |
| Edición concurrente durante una poda manual | `Test_Runner::test_prune_self_persist_does_not_resurrect_a_concurrently_forgotten_item` | A |
| Cambios concurrentes a otras entradas durante `forget()`, un fallo de renderizado registrado o uno resuelto | `Test_State::test_forget_does_not_resurrect_a_different_concurrently_forgotten_item`, `test_record_render_failure_does_not_resurrect_concurrent_changes_to_other_items`, `test_clear_render_failure_does_not_resurrect_concurrent_changes_to_other_items` | A |
| Fallo de renderizado resuelto concurrentemente para una entrada que la ejecución en curso no toca | `Test_Runner::test_concurrent_render_failure_clear_for_an_untouched_item_is_not_resurrected` | A |
