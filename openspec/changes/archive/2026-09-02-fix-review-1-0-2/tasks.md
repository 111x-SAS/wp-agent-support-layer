...
- [x] 20.1 Añadir la sección "Filters" a `readme.txt` (y su resumen en `README.md`) con todos los filtros y acciones listados en D16, incluidos `wpasl_page_after_form`, `wpasl_generation_failed` y `wpasl_max_failures`; verificar que cada nombre listado existe en `src/` con `grep`
- [x] 20.2 Actualizar `docs/spec-coverage.md` con las filas nuevas y modificadas de los ocho deltas (incluidas las once filas que el review marca como parciales) apuntando a los tests creados; verificar que cada test citado existe con `grep -rn "function test_" tests/`
- [x] 20.3 Regenerar `languages/wp-agent-support-layer.pot` con las cadenas nuevas ("Failed items", 503 de `llms-full.txt`, comprobaciones nuevas del diagnóstico) y verificar que `vendor/bin/phpcs` sigue limpio
